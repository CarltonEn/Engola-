...
def status(request:Request):
    ...
    return {'ok':True,'google':{'configured':_cg(),'connected':bool(g),'token_valid':bool(g and g[1] and g[1]>time.time()),'scope':g[0] if g else None,'redirect_uri':config.GOOGLE_REDIRECT_URI or None},'github':{'configured':_ch(),'connected':bool(h),'scope':h[0] if h else None,'redirect_uri':config.GITHUB_REDIRECT_URI or None}}
